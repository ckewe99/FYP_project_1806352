...
@section('content')
    <form method="post"
        action="{{ route('menu.update', [$data->id, 'days' => $data->days, 'session' => $data->session, 'date_range_id' => $data->date_range_id]) }}">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-8 form-group {{ $errors->has('name') ? 'has-danger' : '' }}">
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                        placeholder="{{ __('Please Enter Food Name') }}" value="{{ old('name') ?? $data->name }}"
                        autofocus required>
                    @error('name')
            ...
            <div class="row">
                ...
                <div class="col-md-8 form-group {{ $errors->has('price') ? 'has-danger' : '' }}">
                    <input type="number" name="price" step=".01" class="form-control @error('price') is-invalid @enderror"
                        placeholder="{{ __('Please Enter Price') }}" value="{{ old('price') ?? $data->price }}"
                        autofocus required>
                    @error('price')
                        <div class="invalid-div text-danger">{{ $message }}</div>
                    @enderror
            ...
            {{-- <div class="row">
                ...
                <div class="col-md-8 form-group {{ $errors->has('type') ? 'has-danger' : '' }}">
                    <input type="text" name="type" class="form-control @error('type') is-invalid @enderror"
                        placeholder="{{ __('Please Enter Type') }}" value="{{ old('type') ?? $data->type }}" autofocus>
                    @error('class')
                        <div class="invalid-div text-danger">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <select class="browser-default custom-select" type='text' name='stall'>
                        @foreach ($stalls as $stall)
                            <option value="{{ $stall->id }}"
                                {{ isset($data) ? ($data->stall_id == $stall->id ? 'selected' : '') : '' }}>
                                {{ $stall->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="col-md-8 form-group {{ $errors->has('start') ? 'has-danger' : '' }}">
                <input type="text" name="start" id="flatpickr-input-start"
                    ...
                    placeholder="{{ __('Please Enter Start Date') }}" readonly value="{{ Carbon\Carbon::parse($date_range->start)->format('d-m-Y')}}" autofocus required>
                @error('start')
                    <div class="invalid-div text-danger">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="row">
            ...
            <div class="col-md-8 form-group {{ $errors->has('end') ? 'has-danger' : '' }}">
                <input type="text" name="end" id="flatpickr-input-end"
                    ...
                    placeholder="{{ __('Please Enter End Date') }}" readonly value="{{ Carbon\Carbon::parse($date_range->end)->format('d-m-Y') }}" autofocus required>
                @error('end')
                    <div class="invalid-div text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-8 form-group {{$errors->has('dessert')?'has-danger':''}}">
                ...
                <input type="checkbox" name="dessert" data-toggle="toggle" data-style="ios" data-on="Yes" data-off="No" {{$data->dessert ? 'checked' : ''}}>
                @error('dessert')
                <div class="invalid-div text-danger">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="container">
            <div class="row">
                <div class="col">
                    <a href="{{ route('menu.show', [$data->days, 'date_range_id' => $date_range_id]) }}"
